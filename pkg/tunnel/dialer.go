...
func (h *dialer) connToStreamLoop(ctx context.Context, wg *sync.WaitGroup) {
	...
	defer func() {
		...
		close(outgoing)
		dlog.Debugf(ctx, "   CONN %s conn-to-stream loop ended", id)
		wg.Done()
	}()
	...
	for atomic.LoadInt32(&h.connected) == connected {
		...
		if err != nil {
			if !(errors.Is(io.EOF, err) || errors.Is(net.ErrClosed, err)) {
				dlog.Errorf(ctx, "!! CONN %s, conn read: %v", id, err)
			}
...
func (h *dialer) streamToConnLoop(ctx context.Context, wg *sync.WaitGroup) {
	...
	defer func() {
		wg.Done()
		h.startDisconnect(ctx)
		dlog.Debugf(ctx, "   CONN %s stream-to-conn loop ended", id)
	}()
	...
	for atomic.LoadInt32(&h.connected) != notConnected {
		...
		case dg := <-incoming:
			...
			for n := 0; n < pn; {
				...
				if err != nil {
					h.startDisconnect(ctx)
					dlog.Errorf(ctx, "!! CONN %s, write: %v", id, err)
					return
				}
